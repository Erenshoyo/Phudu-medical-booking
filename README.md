...
> **Tagline:** Dependable Care, Backed by Trusted Professionals.
...
- **DoctorData.json**  
  Contains doctor profiles, availability, fees, etc.

- **blogsData.json**  
  Contains blog titles, authors, and content.
...
- **Availability Logic**
  The `checkAvailability` function in `dateUtils.js` currently contains a syntax error:
  `=>` should be changed to `>=` for correct comparison.

- **Booking Route Loader**
  The `/bookings` route in `Routes.jsx` has a loader attached that attempts to read
  `params.education`, which is undefined on that route.

  👉 The loader can be safely removed since `Bookings.jsx` already handles data via `localStorage`.
